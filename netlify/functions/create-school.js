...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!profile || profile.role !== "admin") {
      return response(403, { error: "Solo gli amministratori possono creare scuole" });
    }

    const { name, city, address, province } = JSON.parse(event.body);
    ...
    const { data, error } = await admin
      .from("schools")
      .insert({ name, city, address, province })
      .select()
      .single();
